...
class FormularioSede(forms.ModelForm):
    ...
    def __init__(self, edicion=False, *args, **kwargs):
        ...
        if not edicion:
            ...
            self.fields['entregadas'].queryset = m.Integrante.objects.exclude(sedes__fecha=datetime.date.today())

    class Meta:
        model = m.Sede
        fields = ('colonia', 'nombre', 'unidades', 'entregadas',)
        ...
        widgets = {
            ...
            'entregadas': forms.SelectMultiple(attrs={'class': 'form-control'}),
        }
...
class FormularioRecibo(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = ('recibo_inicio', 'recibo_final', 'cantidad')
        widgets = {
            'recibo_inicio': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Recibo inicial'}),
            'recibo_final': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'Recibo final'}),
            'cantidad': forms.NumberInput(attrs={'class': 'form-control', 'placeholder': 'Cantidad de enrolados'}),
        }
    ...
    def clean_recibo_inicio(self):
        return self.validar_recibo(self.cleaned_data['recibo_inicio'])

    def clean_recibo_final(self):
        return self.validar_recibo(self.cleaned_data['recibo_final'])
